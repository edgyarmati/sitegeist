...
Ghost in the Web uses a **CORS Proxy** to handle Cross-Origin Resource Sharing restrictions for:
...
- **`src/tools/extract-document.ts`**:
  - Tool has optional `corsProxyUrl` property set by Ghost in the Web if proxy is enabled
...
- **`src/agent/transports/proxy-types.ts`**:
  ...
  - Not used by Ghost in the Web - only `ProviderTransport` is used
...
- **`src/agent/transports/AppTransport.ts`**:
  ...
  - **Not used by Ghost in the Web** - Ghost in the Web only uses `ProviderTransport`
...
- `src/agent/transports/ProviderTransport.ts` - LLM request proxy logic (USED by Ghost in the Web)
- `src/agent/transports/AppTransport.ts` - Alternative transport (NOT used by Ghost in the Web)
- `src/agent/transports/proxy-types.ts` - Event types for AppTransport (NOT used by Ghost in the Web)
